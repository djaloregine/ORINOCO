...
let presenterQuantite = document.createElement("p");
presenterQuantite.classList = "presentation";
presenterQuantite.textContent = "Combien souhaitez vous d'ours en peluche ? "
document.getElementById("testUn").appendChild(presenterQuantite);

let choisirQuantite = document.createElement("buton");
choisirQuantite.classList = "compteur";
document.getElementById("testUn").appendChild(choisirQuantite);



const choix = (number) => {
    for (let i = 1; i < 11; i++) {
        document.getElementsByClassName("compteur").addEventListener("click", function () {
            let number = 0;
            return number += 1;
        })
    }
